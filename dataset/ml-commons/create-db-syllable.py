      ...
      if syllables == None:
        ...
        while syllable_count < 15:
          ...
          if syllable_count <= syllables_count:
            SQL_value = syllables[syllable_count -1]
          else:
            SQL_value = "0"
          if syllable_count < 14:
            SQL_fields = SQL_fields + "syllable" + str(syllable_count) + ", "
            SQL_values = SQL_values + str(SQL_value)  + "', '"
          else:                   
            SQL_fields = SQL_fields + "syllable" + str(syllable_count) + ", sylable) VALUES ("
            SQL_values = SQL_values + str(SQL_value)  + "'," + str(syllables_count) + ")"            
          syllable_count += 1
...
class SyllableTokenizer(TokenizerI):
    ...
    def __init__(self, lang="en", sonority_hierarchy=False):
        ...
        if not sonority_hierarchy and lang == "de":
            sonority_hierarchy = [
                "aeiouy",  # vowels.
                "lmnrw",  # nasals.
                "zvsf",  # fricatives.
